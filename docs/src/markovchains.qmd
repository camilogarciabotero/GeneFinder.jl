...
![DNA sequence as a Markov chain with a DNA alphabet $\mathscr{A} = \{A, C, G, T\}$](../assets/nucleotide-markov-chain.png){#fig-dna-markov fig-align="center"}
...
Note that @eq-seqprobability has two terms, a initial probability $P(X_{1} = i_{1})$ and the the product of all transitions beginning at $t=2$. 
